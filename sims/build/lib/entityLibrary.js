...
import { remap, spiral } from "./utils.js";
const s = spiral({
    angle: Math.PI,
    offset: Math.PI,
    center: { x: 0, y: 5, z: 0 },
});
function sketchbook_page_in_spiral(id) {
    ...
    return {
        ...
        components: {
            ...
            calculateRotation: {
                calculation: (m, e) => {
                    const t = m.input.entityUnderMouse === e.id || e.components.tagActive
                        ? m.time / 2
                        : m.time / 12;
            ...
            calculatePosition: [
                getLerpToPosComponent(target1),
                {
                    calculation: (m, e) => {
                        ...
                        return {
                            y: m.input.entityUnderMouse === e.id || e.components.tagActive
                                ? 4
                                : 0,
                        };
                    },
